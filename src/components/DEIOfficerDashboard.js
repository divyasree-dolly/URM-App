...
const DEIOfficerDashboard = () => {
  ...
  const validateLoginForm = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost/login_dei.php', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        window.location.href = "/DODashboard";
      } else {
  ...
  const validateSignUpForm = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost/register_dei.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
        },
...
export default DEIOfficerDashboard;
